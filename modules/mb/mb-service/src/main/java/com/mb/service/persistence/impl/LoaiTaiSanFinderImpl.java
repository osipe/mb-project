 ...
 * @generated
 */
@Component(service = LoaiTaiSanFinder.class)
public class LoaiTaiSanFinderImpl extends LoaiTaiSanFinderBaseImpl implements LoaiTaiSanFinder {
	@ServiceReference(type = CustomSQL.class)
	private CustomSQL _customSQL;

	public static final String FIND_BASE = LoaiTaiSanFinder.class.getName() + ".findBase";
	public static final String COUNT_BASE = LoaiTaiSanFinder.class.getName() + ".countBase";
	...
	public List<LoaiTaiSan> findBase(String ma, String ten, String ghiChu, int hoatDong, int start,
		...
		try {
			...
			if (Validator.isNull(ghiChu)) {
				sql = sql.replace("AND (LOWER(ghiChu) LIKE ?)", "");
			}
			sql = _customSQL.replaceOrderBy(sql, obc);
			SQLQuery q = session.createSQLQuery(sql);
			q.addEntity("LoaiTaiSan", LoaiTaiSanImpl.class);
			...
			if (Validator.isNotNull(ghiChu)) {
				qPos.add("%" + ghiChu.toLowerCase() + "%");
			}
			return (List<LoaiTaiSan>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int countBase(String ma, String ten, String ghiChu, int hoatDong) throws SystemException {
		Session session = null;
		try {
			session = openSession();

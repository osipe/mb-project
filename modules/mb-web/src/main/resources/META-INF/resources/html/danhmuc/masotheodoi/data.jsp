<%@page import="quanly.portlet.danhmuc.masotheodoi.MaSoTheoDoiComparator"%>
...
<%
	...
	int count = MaSoTheoDoiLocalServiceUtil.countBase(maSearch, dienGiaiSearch, hoatDongSearch);
...
<div id="<portlet:namespace />recordSearchContainer">
	<liferay-ui:search-container  delta="20"  emptyResultsMessage="Không có kết quả nào được tìm thấy!" iteratorURL="<%=iteratorURL %>" total="<%=count %>" >
		 <%
		 	...
		 	MaSoTheoDoiComparator obc = new MaSoTheoDoiComparator(orderByCol,ascending);
		 	searchContainer.setOrderByCol(orderByCol);
		 	searchContainer.setOrderByType(orderByType);
		 	searchContainer.setOrderByComparator(obc);
		 
			List<MaSoTheoDoi> items = MaSoTheoDoiLocalServiceUtil.findBase(maSearch, dienGiaiSearch, hoatDongSearch, searchContainer.getStart(), searchContainer.getEnd(), searchContainer.getOrderByComparator());
		 %>
		 <liferay-ui:search-container-results results="<%= items %>" />
		 <liferay-ui:search-container-row className="com.mb.model.MaSoTheoDoi" modelVar="maSoTheoDoi" keyProperty="maSoTheoDoiId" > 
			<portlet:renderURL var="editURL" windowState="<%=LiferayWindowState.POP_UP.toString()%>">
				<portlet:param name="mvcPath" value="/html/danhmuc/masotheodoi/add.jsp" />
				<portlet:param name="maSoTheoDoiId" value="<%=String.valueOf(maSoTheoDoi.getMaSoTheoDoiId())%>" />
			</portlet:renderURL>
			<portlet:resourceURL var="hoatDongURL" id="hoatDongURL">
				...
				<%
			 	...
			 	%>
			 <liferay-ui:search-container-column-text cssClass="text-center" name="Mã" value="${maSoTheoDoi.ma}" orderable="true" orderableProperty="ma"/>
			 <liferay-ui:search-container-column-text cssClass="text-center" name="Diễn giải" value="${maSoTheoDoi.dienGiai}" orderable="true" orderableProperty="dienGiai"/> 
			  <liferay-ui:search-container-column-text name="Thao tác" cssClass="aui-w10 text-center">
			 	...
			 	<a href="javascript:void(0);" onclick="<%=hoatDongAction %>" title='<%=maSoTheoDoi.getHoatDong() ? "Ngưng hoạt động" : "Hoạt động" %>'>
			 		<span style='<%=maSoTheoDoi.getHoatDong() ? "color:#ff3d00e8" :  "color:blue"%>'><i class='<%=maSoTheoDoi.getHoatDong() ? "glyphicon glyphicon-off" : "glyphicon glyphicon-play-circle"%>'></i></span>
			 	</a>

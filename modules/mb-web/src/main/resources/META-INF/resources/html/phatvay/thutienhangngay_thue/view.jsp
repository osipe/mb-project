	...
	<table class="info table-pa5 aui-w100">
			...
			<tr>
				<td  colspan="2">
					...
					<button id="<portlet:namespace />inPhieuThuHangNgay" type="button" class="btn btn-labeled btn-info" onclick="printBangKeThue();">
						<span class="btn-label"><i class="glyphicon glyphicon-print "></i></span>In Bảng Kê Thuế
					</button>
				</td>
			</tr>
		<tr>
...
AUI().ready(['aui-base'], function(A) {
	...
	Liferay.provide(window,'printBangKeThue', function(){
		var url = '${printBangKeThue}';
		var ngayThuTien = 0;
		var pattern = /(\d{2})\/(\d{2})\/(\d{4})/;
		var ngayThuTienSearchVal = A.one('#<portlet:namespace />ngayThuTienSearch').val();
		if (('' != ngayThuTienSearchVal && pattern.test(ngayThuTienSearchVal)) && 'dd/MM/yyyy' != ngayThuTienSearchVal) {

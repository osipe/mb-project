...
<fieldset class="fieldset-border">
	...
	<div id="idSearchAdvance" class="collapse">
		<table class="info table-pa5 aui-w100">
			<tr>
				<td>
					<aui:input name="soTienVaySearch" class="input-text" label="Số tiền vay">
					</aui:input>
				</td>
				<td>
					<aui:input name="laiSuatVaySearch" class="input-text" label="Lãi suất vay">
					</aui:input>
				</td>
			</tr>
			<tr>
				<td>
					<aui:input name="thoiHanSearch" class="input-text" label="Thời hạn">
					</aui:input>
				</td>
				<td>
...
AUI().ready(['aui-base'], function(A) {
	...
	Liferay.provide(window,'search', function(){
		loadingMask.show();
		var data = {
			'<portlet:namespace/>soTienVaySearch' : A.one('#<portlet:namespace />soTienVaySearch').val(),
			'<portlet:namespace/>laiSuatVaySearch' : A.one('#<portlet:namespace />laiSuatVaySearch').val(),
			'<portlet:namespace/>thoiHanSearch' : A.one('#<portlet:namespace />thoiHanSearch').val(),

...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%
	long chiNhanhId = ParamUtil.getLong(request, "chiNhanhId");
	ChiNhanh chiNhanh = ChiNhanhLocalServiceUtil.createChiNhanh(0L);
	if(chiNhanhId > 0){
		chiNhanh = ChiNhanhLocalServiceUtil.fetchChiNhanh(chiNhanhId);
	}
	if(chiNhanh == null){
		chiNhanh = ChiNhanhLocalServiceUtil.createChiNhanh(0L);
	}
%>
<aui:form name="frmAdd">
	<table class="info table-pa5 aui-w100">
		<tr>
			<td>
				<aui:input class="input-text" name="ma" label="Mã" value="<%=chiNhanh.getMa() %>">
					<aui:validator name="required" errorMessage="Mã không được bỏ trống!" />
				</aui:input></td>
			<td>
				<aui:input name="ten" class="input-text" label="Tên chi nhánh" value="<%=chiNhanh.getTen() %>">
		...
		<tr>
			<td colspan="3">
				<button id="<portlet:namespace />luu" type="button" class="btn btn-labeled btn-default" onclick="save(true);">
					<span class="btn-label"><i class="glyphicon glyphicon-floppy-disk"></i></span><%=chiNhanhId == 0 ? "Lưu" : "Cập nhật" %>
				</button>
				<%
					if(chiNhanhId == 0){
				%>
...
AUI().ready(['aui-base'], function(A) {
	...
	Liferay.provide(window,'save', function(close){
		...
		if(!formValidator.hasErrors()){
			var data = {
				<portlet:namespace />chiNhanhId : '<%=chiNhanhId %>',

...
@Component(immediate = true, property = { "com.liferay.portlet.display-category=category.sample",
		...
		"javax.portlet.init-param.view-template=/html/phatvay/tattoanthue/view.jsp",
		"javax.portlet.display-name=" + QuanlyPortletKeys.tatToanThue, "javax.portlet.name=" + QuanlyPortletKeys.tatToanThue,
...
public class TatToanThuePortlet extends MVCPortlet {
	@Override
	...
	public JSONObject printPhieuTatToan(ResourceRequest resourceRequest, ResourceResponse resourceResponse,
		...
		if (Validator.isNotNull(maCTV)) {
			try {
				...
				in = getServletContext().getResourceAsStream("report/MAU_TAT_TOAN_THUE.docx");
				...
				map.put("LAI_TRA", df.format(0).replaceAll(",", "."));
				map.put("TONG_TIEN", df.format(tongVonTatToan).replaceAll(",", "."));
				map.put("TONG_TIEN_BANG_CHU", DocSo.docSo(GetterUtil.getLong(tongVonTatToan)));
				iContext.putMap(map);

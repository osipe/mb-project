package quanly.portlet.danhmuc.soku;
...
@Component(immediate = true, property = { "com.liferay.portlet.display-category=category.sample",
		...
		"javax.portlet.init-param.view-template=/html/danhmuc/sokheuoc/view.jsp",
		"javax.portlet.display-name=" + QuanlyPortletKeys.soKheUoc, "javax.portlet.name=" + QuanlyPortletKeys.soKheUoc,
...
public class SoKheUocPortlet extends MVCPortlet {
	@Override
	public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		if (resourceId.equals("addOrUpdateURL")) {
			kq = addOrUpdateURL(resourceRequest, resourceResponse, serviceContext);
		}
	...
	public JSONObject addOrUpdateURL(ResourceRequest resourceRequest, ResourceResponse resourceResponse,
			ServiceContext serviceContext) {
		JSONObject kq = JSONFactoryUtil.createJSONObject();
		try {
			long soKheUocId = ParamUtil.getLong(resourceRequest, "soKheUocId");
			String cauTruc = ParamUtil.getString(resourceRequest, "cauTruc");
			int so = ParamUtil.getInteger(resourceRequest, "so");
			SoKheUoc soKheUoc = SoKheUocLocalServiceUtil.createSoKheUoc(0L);
			if (soKheUocId > 0) {
				soKheUoc = SoKheUocLocalServiceUtil.fetchSoKheUoc(soKheUocId);
				soKheUoc.setSo(so);
				soKheUoc.setCauTruc(cauTruc);
				SoKheUocLocalServiceUtil.updateSoKheUoc(soKheUoc, serviceContext);
			} else {
				soKheUoc.setSo(so);
				soKheUoc.setCauTruc(cauTruc);
				SoKheUocLocalServiceUtil.addSoKheUoc(soKheUoc, serviceContext);
			}
		} catch (Exception e) {
			e.printStackTrace();
			kq.putException(e);
